...
const page = () => {
  ...
  const getwisdom = async () => {
    const response = await axios.get(
      `/${locale}/api/information_service/informationServiceBySlug/?submenu_slug=/info_service/language_nation&page=${currentPage}&page_size=${postsPerPage}`
    );
  ...
  const lastPage = async (total) => {
    const response = await axios.get(
      `/${locale}/api/information_service/informationServiceBySlug/?submenu_slug=/info_service/language_nation&page=${total}&page_size=${postsPerPage}`
    );
  ...
  return (
    <div>
      <Head>
        <title>{t("page-titles.info-service.lang-nation")}</title>
            ...
            <ul className="">
              {submenu.map((item) => (
                <li key={item.id} className="bg-[#3A2F7D]">
                  {item.slug === "/info_service/language_nation" ? (
